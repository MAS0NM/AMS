...
class IndexController extends Controller {
    ...
    public function insign(){
        ...
        if (empty($get['empno'])) {
        	//$this -> error('请输入员工号');
        	echo "<span style='color:#DC143C;'>请输入员工号</span>";
        	exit;
        }
		...
		if ($data) {
            ...
            if ($res1) {
            	...
            	echo "<span style='color:#DC143C;'>您今天已签到</span>";
            }else{
            	if ($t > 0) {
					if ($t >= $time) {
	                	$get['status'] = '旷工';
				...
				if ($result) {
					...
					echo "<span style='color:#9ACD32;'>签到成功</span>";
				}else{
					//$this -> error('签到失败');
					echo "<span style='color:#DC143C;'>签到失败</span>";
				}
            }			
		}else{
			//$this -> error('员工号不存在');
			echo "<span style='color:#DC143C;'>员工号不存在</span>";
		}
    }
		...
		if ($data) {
			...
			if ($res3) {
				if ($res1) {
	            	...
	            	echo "<span style='color:#DC143C;'>您今天已签退</span>";
	            }else{
					if ($t > 0) {
						if ($t >= $time) {
		                	$get['status'] = '旷工';
					...
					if ($result) {
						...
						echo "<span style='color:#9ACD32;'>签退成功</span>";
					}else{
						//$this -> error('签退失败');
						echo "<span style='color:#DC143C;'>签退失败</span>";
					}
				}
			}else{
				//$this -> error('请先签到');
				echo "<span style='color:#DC143C;'>今天还没签到</span>";
			}
		}else{
			//$this -> error('员工号不存在');
			echo "<span style='color:#DC143C;'>员工号不存在</span>";
